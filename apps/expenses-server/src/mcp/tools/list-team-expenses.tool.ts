...
const listTeamExpensesInput = {
    ...
    status: z
        .array(z.enum([ExpenseStatus.PENDING, ExpenseStatus.APPROVED, ExpenseStatus.REJECTED, ExpenseStatus.PAID]))
        .optional()
...
Note: You can only view expenses from users who report to you directly (your team).`,
    ...
    handler: async (args, extra) => {
        try {
            ...
            if (user.role !== UserRoles.MANAGER && user.role !== UserRoles.FINANCE_ADMIN) {
            ...
            if (!department && user.role === UserRoles.MANAGER) {
