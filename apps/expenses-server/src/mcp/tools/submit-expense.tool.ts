...
const categoryNames = DEFAULT_CATEGORIES.map(c => c.name) as [string, ...string[]];
...
The expense will be created with 'pending' status and routed to the appropriate manager for approval.`,
    ...
    handler: async (args, extra) => {
        try {
            ...
            const email = authInfo.claims?.email as string | undefined || `${userId}@example.com`;
            const name = authInfo.claims?.name as string | undefined || "Unknown User";
            // Roles might be in claims or scopes.
            // Descope puts roles in 'tenants' claim usually, or we can look at our DB.
